...
def _check_incomplete_approval_gates(db: Session, project_id: str) -> Optional[QAIssue]:
    """G5 check: any open (requested/pending) approval requests block QA gate G5."""
    from app.models.workflow import ApprovalRequest, ApprovalStatus
    open_statuses = [ApprovalStatus.requested.value, ApprovalStatus.pending.value]
    open_approvals = [
        ar.id for ar in db.query(ApprovalRequest)
        .filter(
    ...
    if open_approvals:
        return QAIssue(
            rule="incomplete_approval_gates",
            severity="error",
            message=f"{len(open_approvals)} approval request(s) still open — gate G5 cannot pass.",
            item_ids=open_approvals,
        )
    return None


def _check_scope_finding_mismatch(db: Session, project_id: str) -> Optional[QAIssue]:
    """Findings with categories not present in the approved scope items indicate scope boundary drift."""
    from app.models.scope import ScopeItem
    approved_scope_values = {
        si.value.lower()
        for si in db.query(ScopeItem)
        .filter_by(project_id=project_id, approved=True)
        .all()
    }
    # For VAPT: findings referencing categories whose parent assets aren't in scope
    mismatched = []
    for finding in db.query(Finding).filter_by(project_id=project_id).all():
        if finding.pack_scoped_data:
            target = finding.pack_scoped_data.get("target", "").lower()
            if target and not any(target in sv for sv in approved_scope_values):
                mismatched.append(finding.id)
    if mismatched:
        return QAIssue(
            rule="scope_finding_mismatch",
            severity="warning",
            message=f"{len(mismatched)} finding(s) reference targets not in the approved scope.",
            item_ids=mismatched,
        )
    return None


def _check_report_gaps(db: Session, project_id: str) -> Optional[QAIssue]:
    """All approved findings must have evidence_item_ids and a non-empty description."""
    approved_statuses = {
        FindingStatus.approved.value,
        FindingStatus.client_shared.value,
        FindingStatus.remediation_planned.value,
        FindingStatus.retest_pending.value,
        FindingStatus.closed.value,
        FindingStatus.risk_accepted.value,
    ...
    gap_findings = [
        f.id for f in db.query(Finding)
        .filter(Finding.project_id == project_id, Finding.status.in_(list(approved_statuses)))
        .all()
        if not f.evidence_item_ids or not f.description
    ]
    if gap_findings:
        return QAIssue(
            rule="report_gaps",
            severity="error",
            message=f"{len(gap_findings)} approved finding(s) have missing evidence or description.",
            item_ids=gap_findings,
        )
    return None


def _check_conflicting_client_inputs(db: Session, project_id: str) -> Optional[QAIssue]:
    """Detect scope items where approved=True but a newer approval request exists to change scope."""
    from app.models.scope import ScopeItem
    from app.models.workflow import ApprovalRequest, ApprovalStatus
    open_scope_approvals = (
        db.query(ApprovalRequest)
        .filter(
            ApprovalRequest.project_id == project_id,
            ApprovalRequest.target_type == "scope_item",
            ApprovalRequest.status.in_([ApprovalStatus.requested.value, ApprovalStatus.pending.value]),
        ).all()
    )
    conflict_ids = [ar.target_id for ar in open_scope_approvals]
    if conflict_ids:
        return QAIssue(
            rule="conflicting_client_inputs",
            severity="warning",
            message=(
                f"{len(conflict_ids)} approved scope item(s) have pending change requests — "
                "resolve before final report."
            ),
            item_ids=conflict_ids,
        )
    return None


def _check_finding_without_remediation(db: Session, project_id: str) -> Optional[QAIssue]:
    """Every approved finding (non-informational) must have at least one RemediationAction."""
    non_info_statuses = {FindingStatus.approved.value, FindingStatus.client_shared.value}
    findings_needing = db.query(Finding).filter(
        Finding.project_id == project_id,
        Finding.status.in_(list(non_info_statuses)),
        Finding.severity != "info",
    ).all()
    without = [
        f.id for f in findings_needing
        if not db.query(RemediationAction).filter_by(finding_id=f.id).first()
    ]
    if without:
        return QAIssue(
            rule="finding_without_remediation",
            severity="warning",
            message=f"{len(without)} approved finding(s) have no remediation action defined.",
            item_ids=without,
        )
    return None
...
_RULE_HANDLERS = {
    ...
    # C5 additions
    "incomplete_approval_gates":    _check_incomplete_approval_gates,
    "scope_finding_mismatch":       _check_scope_finding_mismatch,
    "report_gaps":                  _check_report_gaps,
    "conflicting_client_inputs":    _check_conflicting_client_inputs,
    "finding_without_remediation":  _check_finding_without_remediation,
}
